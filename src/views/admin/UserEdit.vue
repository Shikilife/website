<template>
  <div class="p3-user-page">
    ...
    <div class="p3-card p3-form">
      <div class="grid">
        <div class="field">
          <label>學號（userID / readonly）</label>
          <input v-model="form.userID" class="p3-input" readonly />
        </div>

        <div class="field">
          <label>姓名（username）</label>
          <input v-model.trim="form.username" class="p3-input" />
        </div>
        ...
        <div class="field">
          ...
          <div class="p3-dd" :class="{ open: ddGenderOpen }">
            <button type="button" class="p3-dd-btn p3-dd-btn--full" @click.stop="ddGenderOpen = !ddGenderOpen">
        ...
        <div class="field">
          ...
          <div class="p3-dd" :class="{ open: ddClassOpen }">
            ...
            >
              <span class="p3-dd-text">{{ selectedClassText }}</span>
              <span class="p3-dd-caret">▼</span>
            </button>
        ...
        <div class="field">
          ...
          <input v-model.trim="form.phone" class="p3-input" />
        </div>

        <div class="field">
          <label>Email（email）</label>
          <input v-model.trim="form.email" type="email" class="p3-input" />
        </div>

        <div class="field full">
          <label>地址（address）</label>
          <input v-model.trim="form.address" class="p3-input" />
        </div>

        <div class="field">
          <label>密碼（password）</label>
          <input v-model.trim="form.password" type="password" class="p3-input" />
        </div>
        ...
        <div class="field">
          ...
          <div class="p3-dd" :class="{ open: ddAdminOpen }">
            <button type="button" class="p3-dd-btn p3-dd-btn--full" @click.stop="ddAdminOpen = !ddAdminOpen">
      ...
      <div class="actions">
        <button class="p3-btn p3-btn-primary" @click="submit" :disabled="isLoading || !form.userID">
          {{ isLoading ? "儲存中..." : "儲存" }}
        </button>
...
function normalize(row) {
  const cid = row?.classID ?? row?.classId ?? row?.class_id ?? null;
  const classID = cid === "" || cid == null ? null : Number(cid);
...
async function loadUser() {
  ...
  try {
    ...
    const raw = await res.json();
    if (!Array.isArray(raw)) throw new Error("API 回傳不是陣列");

    const u = raw.map(normalize).find((x) => x.userID === userID);
    if (!u) throw new Error(`找不到學號 userID=${userID}`);

    form.value = { ...u };
  } catch (e) {
    errorMsg.value = `載入失敗：${e?.message || "unknown error"}`;
  } finally {
...
async function submit() {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/users/${encodeURIComponent(form.value.userID)}`, {
      method: "PUT",
    ...
    okMsg.value = data?.message || "更新成功";
    setTimeout(() => router.push("/admin"), 350);
  } catch (e) {
    errorMsg.value = `更新失敗：${e?.message || "unknown error"}`;
  } finally {
    isLoading.value = false;
  }
}
